...
const getUserProgress = async (req, res) => {
    const habits = await Habit_1.default.find({ user: req.user?._id });
    ...
    const allDates = habits.flatMap((h) => h.completedDates.map((d) => new Date(d)));
    // Sort ascending
    ...
    for (const d of allDates) {
        ...
        if (!prevDate) {
            ...
            const diff = (new Date(day).getTime() -
